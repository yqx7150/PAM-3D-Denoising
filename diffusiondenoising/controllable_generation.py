...
def get_pc_inpainter(sde, predictor, corrector, inverse_scaler, snr,
  ...
  def pc_inpainter(model, file_path, ckpt_filename, k_w,data):
    ...
    with torch.no_grad():
      ...
      data=torch.from_numpy(data).to(torch.float32).cuda().unsqueeze(0).unsqueeze(0)
      x_mean = x_input
      ...
      print(torch.max(x_input),torch.min(x_input))
      ...
      for i in range(sde.N):
          ...
          print('=====',x_mean.dtype)
          #rint(torch.max(x_mean), torch.min(x_mean))


          ####保真
          n = toNumpy(data).squeeze()
          print('===========',x_mean.shape,z.shape,n.shape)
          w = 1 / (1 * (np.exp(xx / 22)))
          w = np.diag(w)
          hyper = 150
          sum_diff = xx - x_mean
          ...
          x_show = x_show * 255.0
          cv2.imwrite("./out/" + str(i) + ".png", x_show)
          psnr = compare_psnr(x_show, good_input, data_range=255)
          ssim = compare_ssim(x_show, good_input, data_range=255)
          # eng.back_projection("./qq_7.8/testpadsecond_" + str(i) + ".png")
          # back=cv2.imread("/home/lqg/LZL/diffu组会9.16要求/tmp/tmp.png")[:,:,0]
          #cv2.imwrite("./out/testpadsecond_" + str(i) + ".png", x_show)
