...
import io
import likelihood
import controllable_generation_ori as controllable_generation
...
################
# data=375*500*3, datapad = 512*512*3

# data=321*481*3, datapad = 512*512*3
#################
...
for aaa in sorted(os.listdir(path)):
    ...
    mask = io.loadmat('./mask/mask50_p.mat')['mask']
    ...
    print('=====ratio: ', np.sum(mask)/(512*512*3))

    
    #dimg = np.zeros(data.shape).astype(np.float64)    ##**##
    dimg = np.zeros(datapad.shape).astype(np.float64)
      
    #dimg = data*mask  #np.random.uniform(0,1,(256,256,3))*mask   ##**##
    dimg = datapad*mask  #np.random.uniform(0,1,(256,256,3))*mask # #
    ...
    write_Data("./results/result_zero.txt", 'ratio_'+str(np.sum(mask)/(512*512*3)) + '_' + aaa, {"psnr_zero":psnr_zero, "ssim_zero":ssim_zero})
    
    
    Ny,Nx,Nc = dimg.shape
    #dimgp,vid,mid,datap = make_dsr_rgb(dimg,Nimg,Nfir,data)    ##**##
    dimgp,vid,mid,datap = make_dsr_rgb(dimg,Nimg,Nfir,datapad)
