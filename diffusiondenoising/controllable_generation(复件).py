...
def get_pc_inpainter(sde, predictor, corrector, inverse_scaler, snr,
  ...
  def pc_inpainter(model, file_path, ckpt_filename, k_w,y0):
    ...
    with torch.no_grad():
      ...
      for i in range(sde.N):
          ...
          xi=xi.tolist()
          xi=matlab.double(xi)
          net=engine.gradupcompute2(xi,y0)
          bb=net
          bb=np.array(bb)
          xi=bb
          xi=torch.tensor(xi)
          ...
          x_show=x_show*255
          cv2.imwrite('./qq1/testpad_1.png',x_show)
